...
- **Defect_Catalog**：以 Replay_Pipeline 与 Backtest_Pipeline 输出为来源，按 symbol、side、signal type、cycle 聚合得到的策略缺陷清单。
- **Optimization_Proposal**：单条可落地的优化建议，必须包含目标缺陷、影响指标、改动范围、配置开关、回归条件、灰度计划。
...
2. WHEN 生成 Defect_Catalog，THE Optimization 流程 SHALL 记录每条缺陷对应的证据路径、时间区间、trader_id、symbol、signal_id 或 entry_trigger_id、reason_code。
3. IF 某条缺陷无法定位到 decision_logs、replay 报告或 backtest 报告中的具体记录，THEN THE Optimization 流程 SHALL 拒绝接收该条缺陷。
4. WHEN 同一缺陷在多个 trader_id 上重复出现，THE Optimization 流程 SHALL 按 (trader_id, symbol, defect_code) 维度聚合证据，并保留每个 trader 的样本数与方向分布。
...
6. WHEN 生成 Defect_Catalog，THE Optimization 流程 SHALL 输出可机器读取的结构化文件（JSON），字段至少包括：`defect_code`、`description_zh`、`evidence_refs`、`affected_traders`、`affected_symbols`、`affected_sides`、`affected_signal_types`、`primary_metric`、`metric_delta_vs_healthy_subset`。
...
1. THE Baseline_Run SHALL 使用 Backtest_Pipeline 在指定历史区间、指定标的池、指定 config_hash 下跑出，并落盘参数快照、`trades.csv`、`equity.csv`、`signals.csv`、`rejections.csv`。
2. THE Baseline_Run 输出指标 SHALL 至少包含：胜率、Profit_Factor、Net_PnL、Net_PnL_Pct、最大回撤、平均 R、平均持仓时间、信号到执行延迟（信号 close_time 到决策 close_time 到模拟成交时间的两段差值）、手续费占比、滑点占比、拒绝率、按 symbol/side/signal_type 的同口径细分。
3. THE Candidate_Run SHALL 与对应 Baseline_Run 共享同一历史数据 hash、同一 timezone、同一标的池、同一初始资金、同一手续费滑点模型、同一 funding/liquidation 模式。
4. WHEN 对比 Baseline_Run 与 Candidate_Run，THE Optimization_Gate SHALL 计算并记录每个核心指标的绝对值、相对差值与置信区间或 bootstrap 区间。
5. IF Candidate_Run 的最大回撤相对 Baseline_Run 恶化超过配置阈值 `max_drawdown_relative_tolerance`（默认 1.10，即恶化不得超过 10%），THEN THE Optimization_Gate SHALL 拒绝该 Candidate_Run 进入实盘灰度。
6. IF Candidate_Run 的 Profit_Factor 低于 Baseline_Run 的 `profit_factor_relative_floor` 倍（默认 0.95），THEN THE Optimization_Gate SHALL 拒绝该 Candidate_Run 进入实盘灰度。
7. IF Candidate_Run 的拒绝率相对 Baseline_Run 上升超过配置阈值 `rejection_rate_absolute_tolerance`（默认 0.10，绝对值），THEN THE Optimization_Gate SHALL 标记为"信号通过率显著下降"并要求人工复核。
8. WHEN 配置阈值被覆盖，THE Optimization_Gate SHALL 在快照中记录覆盖原因和覆盖人，且不得使用未提交到仓库的本地配置。
9. THE Baseline_Run 和 Candidate_Run SHALL 同时给出分桶指标：按 BTC 市场状态（TREND_UP/TREND_DOWN/RANGING/SQUEEZE/HIGH_VOL）、按 ATR/ADX profile、按 symbol 类别（BTC/ETH/altcoin），以避免整体指标掩盖结构性退化。
...
1. THE Optimization 流程 SHALL 对 Chanlun_Engine 的每一类输出（分型、笔、线段、中枢、`buy1/buy2/buy3`、`sell1/sell2/sell3`、preview、entry trigger）单独输出诊断指标，至少包含：单位时间生成数量、确认延迟（confirm_close_time 与 segment_end_time 之差）、撤回率（确认后又被结构变化推翻的比例）、单信号后续 1R 命中率、单信号后续生命周期最终 R 倍数。
2. WHEN 评估抗噪能力，THE Optimization 流程 SHALL 在历史样本中按 ATR profile（Low/Medium/High）和 ADX 区间（<20、20-30、>30）分别给出上述指标，避免在低波动样本上得到误导性结论。
3. WHEN 优化方案引入新的多周期共振规则（例如 4h 方向 + 1h 主信号 + 15m 触发），THE Optimization_Proposal SHALL 显式声明该规则并提供启用/禁用配置开关，默认值 SHALL 保守，与当前实盘行为一致。
4. WHEN 优化方案引入 ATR/ADX/趋势过滤，THE Optimization_Proposal SHALL 复用现有 `decision/strategy_risk.go` 的 ATR/ADX profile 体系，不得在策略层重复实现并行的 ATR/ADX 判断分支。
5. IF 优化后某一类信号在历史样本上的生成频率下降超过 30%，THEN THE Optimization_Proposal SHALL 在文档中显式说明该频率下降的原因、是否符合预期、对覆盖率的影响。
6. THE Chanlun_Engine 在相同 K 线输入、相同配置、相同状态文件下 SHALL 输出确定性的分型、笔、线段、中枢和买卖点序列；任何优化方案 SHALL 保留该确定性。
7. WHEN 优化方案改变信号去重或 structure_key 计算，THE Optimization 流程 SHALL 与 `programmatic-signal-noise-latency-clarity` 规格保持一致，不得引入新的并行去重逻辑。
...
3. WHEN 优化方案调整分批止盈档位或比例，THE Optimization_Proposal SHALL 验证 `partial_close` 不会触发交易所最小名义额拒绝；若历史样本中存在 `partial_close` 失败，应在 Defect_Catalog 中先行标注。
4. WHEN 优化方案调整移动止损（trailing stop）参数，THE Optimization_Proposal SHALL 与 `.kiro/specs/trailing-stop-optimization/` 规格保持一致，不得新增并行移动止损实现。
5. THE Optimization_Proposal 调整止损 SHALL 使用 `CancelStopLossOrders()` 路径；调整止盈 SHALL 使用 `CancelTakeProfitOrders()` 路径；存在混用即视为违例。
6. WHEN 优化方案引入新的失效条件解析规则，THE Optimization_Proposal SHALL 复用 `decision/parser.go`，不得在策略层旁路。
7. THE Optimization 流程 SHALL 给出 Position_Lifecycle 级别的最大账户回撤、单 Position_Lifecycle 最大回撤、连续亏损次数和回撤恢复时间，作为持仓管理优化的核心评估指标。
...
6. THE Optimization 流程 SHALL 输出"高回撤区间归因"：把超过配置阈值的回撤区间反查到具体 Position_Lifecycle、信号类型和 BTC 状态，便于后续优化定位。
...
2. WHEN 跑 Baseline_Run 与 Candidate_Run，THE Optimization 流程 SHALL 注入虚拟时钟，所有依赖当前真实时间的逻辑 SHALL 使用回测时间或显式禁用并在报告中声明。
3. THE Replay_Pipeline 与 Backtest_Pipeline SHALL 共享 Position_Lifecycle、Execution_Event、信号类型、拒绝原因的字段语义和编码；同一字段在两侧含义不一致即视为违例。
4. WHEN 优化方案上线前，THE Optimization 流程 SHALL 至少完成：对最近一段 Replay_Pipeline 样本的离线复盘对账（不引入未来函数）、Backtest_Pipeline 在相同样本上的 Baseline_Run、Candidate_Run、Optimization_Gate 评估，三者样本范围与 hash SHALL 全部记录在 Optimization_Proposal 中。
5. WHEN 历史区间或标的池变化，THE Optimization 流程 SHALL 以新区间重新生成 Baseline_Run，不得跨区间复用旧基线。
6. THE Optimization_Proposal SHALL 在仓库中保留可机器读取的快照文件（位于 git ignored 的 `backtest_runs/<run_id>/` 内），并在 spec 文档中引用 run_id 与数据 hash。
7. IF Replay_Pipeline 与 Backtest_Pipeline 在同一历史区间得出的同口径指标差异超过配置阈值 `replay_backtest_consistency_tolerance`（默认 0.05 绝对值），THEN THE Optimization 流程 SHALL 在文档中先行说明差异来源，并暂停结论性判断。
...
2. WHEN 优化方案需要新的最小名义额、精度或费率参数，THE Optimization_Proposal SHALL 通过 `trader/exchange_calibration.go` 现有校准入口表达，不得在策略层硬编码交易所差异。
...
5. IF 优化方案在某一交易所实现下使最小名义额拒绝率显著上升，THEN THE Optimization_Gate SHALL 单独按 trader 维度评估而不是只看全局。
...
1. WHEN 优化后的策略输出 Risk_Increase_Action 或 Risk_Reduction_Action，THE 决策日志 SHALL 写入：`structure_key`、`signal_id`、`parent_signal_id`、`entry_trigger_id`、`source_layer`、`signal_type`、`analysis_timeframe`、`trigger_timeframe`、`structure_target`、`signal_close_time`、`decision_close_time`、`age_candles`、`freshness_state`、`reason_code`、`config_hash`、`strategy_version`。
2. THE Replay_Pipeline SHALL 能基于上述字段在离线状态下重建当时的缠论结构上下文（最近笔/线段/中枢编号、ZG/ZD、A/B/C 段边界）。
3. WHEN 优化方案改变决策日志字段，THE Optimization_Proposal SHALL 同步更新 `logger/decision_logger.go` schema、Replay_Pipeline 解析器、`api/server.go` 暴露的 trader-scoped 接口，并保持向后兼容（旧记录使用 `omitempty` 默认值）。
4. WHEN 优化方案改变前端可见字段，THE Optimization_Proposal SHALL 同步更新 `web/src/lib/api.ts` 与 `web/src/types/index.ts`。
5. THE Optimization 流程 SHALL 在每次 Optimization_Gate 通过后，在 spec 任务中记录从 decision_logs 抽样若干笔执行/拒绝事件并人工复盘的结果。
6. IF 优化方案使某类信号在 decision_logs 中无法定位到唯一 (structure_key, entry_trigger_id, action) 组合，THEN THE Optimization_Gate SHALL 拒绝该方案进入实盘灰度。
...
7. FOR ALL 决策日志中的优化后 Risk_Increase_Action 或 Risk_Reduction_Action，THE 日志 SHALL 包含 Requirement 11.1 中列出的所有必填字段；缺失即视为违例。
...
1. THE Optimization 流程 SHALL 为 Requirement 12 中每条 correctness property 提供至少一个 `gopter` 属性基测试。
