...
// 对任意非空的 TradeOutcome 列表，exportTradeHistoryCSV 生成的 CSV 字符串应包含完整表头，
// 且数据行数等于输入列表长度
// 验证: 需求 13.13
...
const tradeOutcomeArb = fc.record({
  ...
  event_type: fc.constantFrom('full_close', 'partial_close', 'auto_close'),
  is_partial: fc.boolean(),
  close_quantity: fc.double({ min: 0, max: 10000, noNaN: true }),
  requested_close_percentage: fc.double({ min: 0, max: 100, noNaN: true }),
  executed_close_percentage: fc.double({ min: 0, max: 100, noNaN: true }),
  order_id: fc.integer({ min: 0, max: 999999999 }),
  signal_id: fc.string({ maxLength: 24 }),
  pnl_source: fc.constantFrom('exchange', 'estimated', ''),
  reconciled: fc.boolean(),
  reconciliation_status: fc.constantFrom('matched', 'pending', 'unsupported', 'estimated_from_decision_log', ''),
  close_reason: fc.string({ maxLength: 80 }).filter((s) => !s.includes('\n') && !s.includes('\r')),
});

describe('Property 48: CSV 导出字段完整性', () => {
  it('对任意非空 TradeOutcome 列表，CSV 应包含完整表头且数据行数等于输入长度', () => {
    fc.assert(
      fc.property(
        ...
        (trades) => {
          ...
          expect(headerColumns).toHaveLength(CSV_HEADERS.length);
          ...
          for (const line of dataLines) {
            ...
            expect(cols).toBe(CSV_HEADERS.length - 1);
          }
        },
      ),
      { numRuns: 100 },
